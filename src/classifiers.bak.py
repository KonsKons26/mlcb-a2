    ...
    def get_hyperparameter_space(self, cv_round):
        ...
        if self.model_type == "LogisticRegression":
            return {
                ...
                "l1_ratio": FloatDistribution(0.001, 0.999),
...
class NestedCV:
    ...
    def _objective(self, trial, X, y, cv_round):
        ...
        # Convert optuna distributions to suggest calls
        params = {}
        space = self.classifier.get_hyperparameter_space(cv_round=cv_round)
        for k, v in space.items():
            ...
            if isinstance(v, CategoricalDistribution):
            ...
            elif isinstance(v, IntDistribution):
                ...
                raise ValueError(f"Unsupported distribution type for {k}")
...
def pipeline(
    ...
    val_set_n = int(n * validation_set_frac)
    val_set = df_without_nans.sample(n=val_set_n, random_state=seed)

    df_without_nans.drop(val_set.index)
    ...
    for model in VALID_MODELS.keys():
        clf = Classifier(model, df, "diagnosis", models_dir=models_dir)
        ncv = NestedCV(clf)
        results = ncv.run()
        print(model)
        print(results)
...
if __name__ == "__main__":
    ...
    pipeline(df=df, target="diagnosis", models_dir=models_dir)